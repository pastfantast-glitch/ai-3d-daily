...
for date in dates:
    ...
    if date==dates[-1]: views.insert(0,('home',ROOT/'index.html'))
    seen={}
    for name,path in views:
        ...
        for card in cards:
            ...
            if not details:
                errors.append(f'{date} {name}: {rid} missing Full Analysis details'); continue
            body=details.find('div',class_='detail-body')
            if not body:
                errors.append(f'{date} {name}: {rid} missing detail-body'); continue
            blocks=recs[rid].get('full_analysis',[])
            headings=body.find_all('h4',recursive=False); paragraphs=body.find_all('p',recursive=False)
            ...
            if rendered!=expected: errors.append(f'{date} {name}: {rid} rendered Full Analysis differs from canonical data')
    ...
    if len(views)==2:
        for rid in recs:
            if seen.get('home',{}).get(rid)!=seen.get('daily',{}).get(rid): errors.append(f'{date}: Full Analysis drift for {rid}')
