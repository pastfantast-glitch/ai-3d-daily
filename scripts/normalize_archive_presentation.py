...
def normalize_quick_impact_structure(soup, impact):
    ...
    if impact.find_all('span', recursive=False): return
...
def normalize_section_header(soup, section, kicker):
    ...
    inner=soup.new_tag('div'); label=soup.new_tag('span', attrs={'class':'section-kicker'}); label.string=kicker
...
def normalize_home_top_card_structure(soup, card, rank):
    ...
    if not rank_node:
        rank_node=soup.new_tag('div', attrs={'class':'top-no'}); card.insert(0, rank_node)
...
def normalize_homepage() -> bool:
    ...
    container=today.select_one('.top-list') or today.select_one('.today-grid'); add_class(container,'top-list')
...
def normalize_top_card_structure(soup, card, rank):
    add_class(card,'daily-card','daily-card-top'); remove_classes(card,'important')
    news_main=card.find('div', class_='news-main', recursive=False); rank_node=card.find('div', class_='news-rank', recursive=False)
    if not news_main:
    ...
    if not rank_node:
        rank_node=soup.new_tag('div', attrs={'class':'news-rank'}); card.insert(0,rank_node)
    rank_node.string=f'{rank:02d}'
...
def normalize_more_card_structure(soup, card):
    remove_classes(card,'news','important','daily-card-top'); add_class(card,'category-news','daily-card','daily-card-more')
...
def normalize(path: Path) -> bool:
    ...
    for card in soup.select('[data-intel-role="card"]'):
        for pill in card.select('.pill'):
            pill['class']=[c for c in (pill.get('class') or []) if c not in LEGACY_PILL_COLORS]; add_class(pill,'pill')
