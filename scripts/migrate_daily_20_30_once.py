...
if old in s: s=s.replace(old,new)
s=s.replace('config-driven target-fill IA','config-driven daily 20-30 release-gate IA')
p.write_text(s,'utf-8')
...
def item(cfg,rank,cat):
    top=int(cfg['homepage']['top5']); nxt=int(cfg['homepage']['next10']); tier='top5' if rank<=top else ('next10' if rank<=top+nxt else 'category_only')
...
for x in d.get('items',[]):
    if x.get('source_url')=='https://www.cgchannel.com/2026/08/nekki-releases-cascadeur-2026-2-with-animation-layers/': x['id']='cascadeur-2026-2-layers'
dp.write_text(json.dumps(d,ensure_ascii=False,separators=(',',':'))+'\n','utf-8')
print('20-30 migration edits applied; visual refresh required before historical regression')
