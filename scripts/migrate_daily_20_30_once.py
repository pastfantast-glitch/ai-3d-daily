...
# ---- Config: daily total release gate, categories are diversity diagnostics ----
...
# ---- Registry normalizer: total daily gate after dedupe ----
...
if old not in s: raise SystemExit('registry exit block not found')
s=s.replace(old,new)
p.write_text(s,'utf-8')
...
def fixture(cfg,n):
    cats=cfg['categories']; return {'date':cfg['collection_contract_effective_date'],'schema_version':int(cfg.get('schema_version',3)),'metadata':{},'items':[item(cfg,r,cats[(r-1)%len(cats)]) for r in range(1,n+1)]}
...
# ---- IA/release/pipeline hardcoded category quotas ----
...
md['category_pool_min_items']=0
md['category_pool_target_items']=5
md['category_pool_max_items']=30
md['category_fill_policy']='daily_total_20_30_quality_first'
dp.write_text(json.dumps(d,ensure_ascii=False,separators=(',',':'))+'\n','utf-8')

print('20-30 migration edits applied; render-before-release-input and date-scoped visual fixes applied')
