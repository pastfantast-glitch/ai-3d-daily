...
def known_identity():
    ...
    for path in sorted((ROOT/'data'/'daily').glob('20??-??-??.json')):
        ...
        for rid,cfg in (data.get('visual_evidence') or {}).items():
            ...
            if source:
                source_to_id.setdefault(source,rid); source_dates.setdefault(source,[]).append(date)
...
def render_details(soup,blocks):
    details=soup.new_tag('details'); summary=soup.new_tag('summary'); summary.string='完整分析'; details.append(summary)
    body=soup.new_tag('div'); body['class']=['detail-body']
    for block in blocks:
...
def main():
    ...
    for card in cards:
        ...
        title=norm(heading.get_text(' ',strip=True)); source_url=norm_url(source.get('href'))
        rid=source_to_id.get(source_url) or generated_id(source_url); source_to_id[source_url]=rid
        in_top=card.find_parent('section',id='top') is not None; blocks=extract_blocks(card)
        card['data-intel-id']=rid; card['data-intel-role']='card'; source['rel']='noopener noreferrer'; source['target']='_blank'
        old=card.find('details'); new=render_details(soup,blocks); old.replace_with(new) if old else source.insert_before(new)
        if rid not in records:
            rec={'id':rid,'slot':'top' if in_top else 'more','full_analysis':blocks,'source_url':source_url}
            prior=sorted(d for d in source_dates.get(source_url,[]) if d<date)
            if prior:
                rec['status']='UPDATE'; rec['delta']=f'Historical backfill: same canonical source already appeared on {prior[-1]}; this record preserves the archived {date} snapshot.'
            records[rid]=rec; order.append(rid)
            visuals[rid]={'enabled':True,'source_url':source_url,'label':'HISTORICAL SOURCE VISUAL','confidence':'medium','keywords':keywords(title)}
    ...
    page.write_text(text,'utf-8'); print(f'backfilled {date}: {len(canonical_ids)} canonical intelligence IDs; TOP={len(top_ids)}')
    return 0
