...
for i,d in enumerate(dirs):
    ...
    header=soup.select_one('header.site-head, header.daily-hero'); main=soup.select_one('main.page'); need(not header,'historical page hero must be removed above sticky navigation'); need(bool(main and 'daily-main' in main.get('class',[])),'missing shared daily-main presentation class')
    history_links=soup.select('nav.global-category-nav a.global-history-link[data-global-view="history"][href="../history/"]')
    need(len(history_links)==1,f'daily archive must expose exactly one History portal link, got {len(history_links)}')
...
print(f'DAILY CONTRACT PASS: {len(dirs)} reports / V2 variable TOP5+next10 + History portal nav + no-hero historical shell + canonical card structure')
