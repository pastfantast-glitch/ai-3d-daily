(()=>{
  if(window.__ai3dCloudSyncV1)return;
  window.__ai3dCloudSyncV1=true;
  ...
  const STYLE_ID='ai3d-cloud-sync-style-v1';
  let busy=false;
  let pushTimer=0;
  ...
  async function register(){
    ...
    try{
      ...
      localStorage.removeItem(DIRTY_STORE);
      paint('已同步');
    }catch(err){console.warn('AI3D cloud sync register failed',err);paint('同步失敗');}
    finally{busy=false;paint();}
  }
  async function pull({reload=true}={}){
    const c=credentials();if(!c||busy)return;busy=true;paint('同步中…');
    try{
      if(localStorage.getItem(DIRTY_STORE)==='1'){await pushNow();return;}
      const out=await call({action:'pull',...c});
      const changed=applyRemote(out.state);
      paint('已同步');
      if(changed&&reload)location.reload();
    }catch(err){console.warn('AI3D cloud sync pull failed',err);paint('離線');}
    finally{busy=false;paint();}
  }
  async function pushNow(){
    ...
    }catch(err){localStorage.setItem(DIRTY_STORE,'1');console.warn('AI3D cloud sync push failed',err);paint('待同步');throw err;}
  }
  function queuePush(){
    if(!credentials())return;
    localStorage.setItem(DIRTY_STORE,'1');
    clearTimeout(pushTimer);pushTimer=setTimeout(()=>pushNow().catch(()=>{}),500);
    paint('同步中…');
  }
  async function importCode(){
    ...
    busy=true;paint('驗證同步碼…');
    try{
      const out=await call({action:'pull',...c});
      setCredentials(c);localStorage.removeItem(DIRTY_STORE);
      applyRemote(out.state);alert('同步完成，這台裝置已連到同一份收藏與偏好資料。');location.reload();
  ...
  async function menu(){
    ...
    }else if(choice==='2'){await pull();}
    else if(choice==='3'){await importCode();}
    else if(choice==='4'&&confirm('只會關閉這台裝置的雲端同步；本機收藏不會刪除。確定嗎？')){setCredentials(null);localStorage.removeItem(DIRTY_STORE);paint();}
  }
