(()=>{
  ...
  import(new URL('./cloud-sync-owner.js?v=20260915-owner-v1',import.meta.url).href).catch(err=>console.warn('Cloud sync unavailable',err));
  ...
  const SAVED_URL=new URL('./saved/',import.meta.url).href;
  ...
  function emptyBookmarks(){return{schemaVersion:1,items:{},updatedAt:null};}
  ...
  function loadBookmarks(){
    try{
      const raw=JSON.parse(localStorage.getItem(BOOKMARK_STORE)||'null');
      if(raw&&Number(raw.schemaVersion)===1&&raw.items&&typeof raw.items==='object')return Object.assign(emptyBookmarks(),raw);
    }catch(_){}
  ...
  function saveBookmarks(){
    ...
    try{localStorage.setItem(BOOKMARK_STORE,JSON.stringify(bookmarks));}catch(_){}
    updateBookmarkNav();
    window.dispatchEvent(new CustomEvent('ai3d:bookmark-change',{detail:{bookmarks:bookmarkList()}}));
  }
  function injectStyle(){
    ...
    const style=document.createElement('style');style.id=STYLE_ID;style.textContent=`
      ...
      .preference-vote button:hover{border-color:#5f58a8;background:#172238;color:#e5e9f2}.preference-vote button.is-up{background:#183526;border-color:#2d6a4d;color:#80e0b2}.preference-vote button.is-down{background:#3a2024;border-color:#6b333b;color:#ff9aa7}.preference-vote button.is-saved{background:#3b3317;border-color:#7f6b2d;color:#ffd866}
      .preference-vote button:focus-visible,.preference-bookmark-link:focus-visible{outline:2px solid #8c80ff;outline-offset:2px}.preference-vote .preference-state{min-width:0;color:#748198;font-size:.68rem}
      .preference-bookmark-link{display:inline-flex;align-items:center;gap:6px;text-decoration:none;white-space:nowrap}.preference-bookmark-count{display:inline-grid;place-items:center;min-width:20px;height:20px;padding:0 5px;border:1px solid rgba(255,216,102,.34);border-radius:999px;color:#ffd866;font-size:.68rem;line-height:1}
      .preference-bookmark-link.is-active{color:#ffd866}
      @media(max-width:760px){.preference-vote{gap:6px}.preference-vote button{height:29px;min-width:33px;padding:0 8px}.preference-bookmark-link .preference-bookmark-label{display:none}}
  ...
  function summaryFor(card){return clean(card.querySelector('.summary')?.textContent);}
  function reportDate(){return clean(document.body?.dataset.reportDate)||clean(document.querySelector('.week-asof')?.textContent)||clean(document.querySelector('.topline span:last-child')?.textContent);}
  function reportUrl(date){return date?new URL(`./${date}/`,import.meta.url).href:location.href;}
  function subcategoryFor(card){return clean(card.querySelector('.pill')?.textContent);}
  function stateText(vote,saved){
    const states=[];
    if(vote===1)states.push('已提高這類內容權重');
    if(vote===-1)states.push('已降低這類內容權重');
    if(saved)states.push('已收藏');
    return states.join(' · ');
  }
  function bookmarkSnapshot(card){
    const id=clean(card.dataset.intelId),date=reportDate();
    return{id,title:titleFor(card),summary:summaryFor(card),source:sourceFor(card),category:currentCategory(card),subcategory:subcategoryFor(card),reportDate:date,reportUrl:reportUrl(date),savedAt:new Date().toISOString()};
  }
  function bookmarkList(){return Object.values(bookmarks.items||{}).sort((a,b)=>String(b.savedAt||'').localeCompare(String(a.savedAt||'')));}
  function isBookmarked(id){return !!bookmarks.items?.[id];}
  function paintCard(card){
    const id=clean(card.dataset.intelId);const wrap=card.querySelector('.preference-vote');if(!id||!wrap)return;
    const vote=Number(profile.votes[id]?.vote)||0;const saved=isBookmarked(id);
    wrap.querySelector('[data-v="1"]')?.classList.toggle('is-up',vote===1);
    wrap.querySelector('[data-v="-1"]')?.classList.toggle('is-down',vote===-1);
    const star=wrap.querySelector('[data-bookmark]');if(star){star.classList.toggle('is-saved',saved);star.textContent=saved?'★':'☆';star.setAttribute('aria-pressed',saved?'true':'false');star.title=saved?'取消收藏':'收藏這篇文章';}
    const state=wrap.querySelector('.preference-state');if(state)state.textContent=stateText(vote,saved);
  }
  ...
  function toggleBookmark(card){
    const id=clean(card.dataset.intelId);if(!id)return;
    if(isBookmarked(id))delete bookmarks.items[id];else bookmarks.items[id]=bookmarkSnapshot(card);
    saveBookmarks();paintId(id);
  }
  function removeBookmark(id){
    id=clean(id);if(!id||!isBookmarked(id))return false;
    delete bookmarks.items[id];saveBookmarks();paintId(id);return true;
  }
  function makeUI(card){
    ...
    wrap.innerHTML='<span>這篇：</span><button type="button" data-v="1" aria-label="喜歡這類內容" title="提高未來同類情報權重">👍</button><button type="button" data-bookmark="1" aria-label="收藏這篇文章" aria-pressed="false" title="收藏這篇文章">☆</button><button type="button" data-v="-1" aria-label="少看這類內容" title="降低未來同類情報權重">👎</button><span class="preference-state" aria-live="polite"></span>';
    wrap.querySelectorAll('[data-v]').forEach(btn=>btn.addEventListener('click',()=>setVote(card,Number(btn.dataset.v))));
    wrap.querySelector('[data-bookmark]')?.addEventListener('click',()=>toggleBookmark(card));
  ...
  function updateBookmarkNav(){
    document.querySelectorAll('.preference-bookmark-count').forEach(node=>node.textContent=String(bookmarkList().length));
  }
  function injectBookmarkNav(root=document){
    root.querySelectorAll?.('.global-category-nav-inner').forEach(inner=>{
      let link=inner.querySelector('.preference-bookmark-link');
      if(!link){
        link=document.createElement('a');link.className='global-category-link preference-bookmark-link';link.href=SAVED_URL;link.dataset.preferenceBookmarkNav='1';link.innerHTML='<span aria-hidden="true">★</span><span class="preference-bookmark-label">收藏</span><b class="preference-bookmark-count" aria-label="收藏數量">0</b>';inner.append(link);
      }
      link.classList.toggle('is-active',new URL(location.href).pathname.includes('/saved/'));
    });
    updateBookmarkNav();
  }
  function enhance(root=document){root.querySelectorAll?.(CARD_SELECTOR).forEach(makeUI);if(root.matches?.(CARD_SELECTOR))makeUI(root);injectBookmarkNav(root);}
