document.addEventListener('DOMContentLoaded',()=>{
  ...
  cards.forEach(card=>{
    const analyses=[...card.querySelectorAll('details')].filter(d=>(d.querySelector('summary')?.textContent||'').trim().includes('完整分析'));
  ...
  analyses.forEach(d=>d.addEventListener('toggle',()=>{
    if(d.open)analyses.forEach(other=>{if(other!==d)other.open=false;});
  }));

  // Lightweight local preference voting. No account and no public counts.
  const STORE='ai3d-preferences-v1';
  let pref={votes:{},tags:{}};
  try{
    pref=Object.assign(pref,JSON.parse(localStorage.getItem(STORE)||'{}'));
    pref.votes=pref.votes||{};
    pref.tags=pref.tags||{};
  }catch(_){ }
  ...
  const tagsFor=card=>{
    ...
    const inferred=[];
    ['BLENDER','UNITY','UNREAL','CHARACTER','ENVIRONMENT','MATERIAL','SHADER','NPR','TOON','AI 3D','RIG','ANIMATION','SUBSTANCE','GEOMETRY NODES'].forEach(t=>{
      if(title.includes(t)||pills.some(p=>p.toUpperCase().includes(t)))inferred.push(t);
    });
  ...
  cards.forEach(card=>{
    ...
    if(!title)return;
    const id=hash(source||title),tags=tagsFor(card),wrap=document.createElement('div');
    wrap.className='preference-vote';
    ...
    const paint=()=>{
      const v=pref.votes[id]?.vote||0;
      wrap.querySelector('[data-v="1"]').classList.toggle('is-up',v===1);
      wrap.querySelector('[data-v="-1"]').classList.toggle('is-down',v===-1);
    };
    wrap.querySelectorAll('button').forEach(btn=>btn.addEventListener('click',()=>{
      ...
      if(vote){
        tags.forEach(t=>pref.tags[t]=(pref.tags[t]||0)+vote);
        pref.votes[id]={vote,tags,title,source,updatedAt:new Date().toISOString()};
      }else delete pref.votes[id];
      Object.keys(pref.tags).forEach(t=>{if(!pref.tags[t])delete pref.tags[t];});
      save();paint();
    }));
    const anchor=card.querySelector('a.source');
    if(anchor)anchor.insertAdjacentElement('afterend',wrap);else card.appendChild(wrap);
    paint();
  });
